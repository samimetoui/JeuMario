...
import com.jeu_mario.Affichage.CompteARebours;
import com.jeu_mario.Affichage.Score;
import com.jeu_mario.Audio.Audio;
import com.jeu_mario.Objets.Bloc;
import com.jeu_mario.Objets.Objet;
import com.jeu_mario.Objets.Piece;
import com.jeu_mario.Objets.TuyauRouge;
import com.jeu_mario.Personnages.Champ;
import com.jeu_mario.Personnages.Mario;
import com.jeu_mario.Personnages.Tortue;
...
public class Scene extends JPanel {
 ...
 public Scene() {
  ...
  this.xFond1 = -50;  /*First background image*/

  this.xFond2 = 750;  /*Second background image*/
...
//  tortue = new Tortue(900, 245);
 ...
 public void paintComponent(Graphics g) {
  ...
  //Détection des contact des personnages avec les objets 
  ...
  //Détection des contact avec les pièces
  ...
  //Contact entre les personnages hors Mario
  for (int i = 0; i < this.tabChamps.size(); i++) {
   for (int j = 0; j < this.tabChamps.size(); j++) { //Test les contacts champignons entre eux
    if (j != i) {
     if (this.tabChamps.get(j).proche(tabChamps.get(i))) {
      this.tabChamps.get(j).contact(tabChamps.get(i));
     }
    }
   }
   for (int j = 0; j < this.tabTortues.size(); j++) { //Test les contacts champignons tortue
    if (this.tabTortues.get(j).proche(tabChamps.get(i))) {
     this.tabTortues.get(j).contact(tabChamps.get(i));
    }
   }
  }

  for (int i = 0; i < this.tabTortues.size(); i++) {
   for (int j = 0; j < this.tabChamps.size(); j++) { //Test les contacts champignons tortue
    if (this.tabChamps.get(j).proche(tabTortues.get(i))) {
     this.tabChamps.get(j).contact(tabTortues.get(i));
    }
   }
   for (int j = 0; j < this.tabTortues.size(); j++) {
    if (j != i) {
     if (this.tabTortues.get(j).proche(tabTortues.get(i))) { //Test les contacts tortues entre elles
      this.tabTortues.get(j).contact(tabTortues.get(i));
     }
    }
   }
  }

  //Contact de mario avec les personnages champ et tortue
  for (int i = 0; i < tabChamps.size(); i++) {
   if (this.mario.proche(tabChamps.get(i)) && this.tabChamps.get(i).isVivant() == true) {
  ...
  for (int i = 0; i < tabTortues.size(); i++) {
   if (this.mario.proche(tabTortues.get(i)) && this.tabTortues.get(i).isVivant() == true) {
  ...
  //Déplacelent de tous les objets fixes
  ...
  //g2.drawImage(this.mario.marche("mario", 25), 300, 245, null); //Mario tempo
  ...
  g2.setFont(police);
  g2.drawString(this.score.getNbrePieces() + " pièce(s) trouvée(s) sur" + this.score.getNBR_TOTAL_PIECES(), 460, 25);
